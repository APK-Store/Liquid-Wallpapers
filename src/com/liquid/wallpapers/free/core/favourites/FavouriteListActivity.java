...
public class FavouriteListActivity extends Activity {
	...
	private class InitFavouriteListTask extends
			LongTimeRunningOperation<List<Favourite>> {
		...
		public void afterOperationSuccessfullyCompleted(List<Favourite> result) {
			if (result.size() < 1) {
				AlertDialogFactory.showInfoMessage(this.context,
						R.string.infoText, R.string.noFavouritesDefinedText);
		...
		public List<Favourite> onRun() throws Exception {
			...
			for (Favourite favourite : favouriteDAO.getAll()) {
				Wallpaper wallpaper = wallpaperManager
						.getWallpaperById(favourite.getWallpaperId());
	...
	private void initListView() {
		ListView listView = (ListView) this
				.findViewById(R.id.favouritesListView);
		...
		listView.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
				...
				selectedFavourite = (Favourite) parent
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
					int position, long id) {
				Favourite favourite = (Favourite) parent
						.getItemAtPosition(position);

				WallpaperPreviewActivity.showPreviewActivity(parentActivity,
						favourite.getWallpaper());
			}
		});
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setTitle(String.format(
				Messages.getString("FavouriteListActivity.0"), //$NON-NLS-1$
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.app.Dialog#onCreateContextMenu(android.view.ContextMenu,
	...
	private synchronized void updateFavouritesAdapter() {
		ListView listView = (ListView) this
				.findViewById(R.id.favouritesListView);
		listView.setAdapter(new FavouriteListAdapter(favourites, this,
