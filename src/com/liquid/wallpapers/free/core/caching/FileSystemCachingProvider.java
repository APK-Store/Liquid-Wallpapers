 ...
 */
package com.liquid.wallpapers.free.core.caching;
...
public final class FileSystemCachingProvider implements IPersistentCache {
	...
	private static final Pattern URI_FILENAME_PATTERN = Pattern
			.compile(Messages.getString("FileSystemCachingProvider.0")); //$NON-NLS-1$
	 ...
	 */
	@Inject
	FileSystemCachingProvider(Context context,
		...
		for (int i = 0; (i < files.size()) && (remaining > 0); i++) {
			...
			if (!this.context.deleteFile(file.getName())) {
				throw new IOException(String.format(
						Messages.getString("FileSystemCachingProvider.1"), //$NON-NLS-1$
						file.getName()));
			}
		}
	}
	 ...
	 * 
	 * @see
	 * de.dan_nrw.android.scroid.core.caching.IPersistentCache#get(java.net.URI,
	 * java.lang.String)
	 */
	@Override
	public Bitmap get(URI uri, String prefix) throws IOException {
		...
		try {
			fileInputStream = this.context.openFileInput(this
					.getFileNameForUri(uri, prefix));
			...
			if (fileInputStream != null) {
				try {
					...
					// nothing to do - exception would hide original exception
					// thrown in try block
				}
			}
		}
	...
	private String getFileNameForUri(URI uri, String prefix) {
		...
		if (!matcher.find()) {
			throw new IllegalArgumentException(
					Messages.getString("FileSystemCachingProvider.2")); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @see
	 * de.dan_nrw.android.scroid.core.caching.IPersistentCache#isInCache(java
	 * .net.URI, java.lang.String)
	 */
	@Override
	public boolean isInCache(URI uri, String prefix) {
		File file = this.context.getFileStreamPath(this.getFileNameForUri(uri,
				prefix));
	 ...
	 * 
	 * @see
	 * de.dan_nrw.android.scroid.core.caching.IPersistentCache#put(android.graphics
	 * .Bitmap, java.net.URI, java.lang.String)
	 */
	@Override
	public void put(Bitmap bitmap, URI uri, String prefix) throws IOException {
		...
		try {
			stream = this.context.openFileOutput(
					this.getFileNameForUri(uri, prefix), Context.MODE_PRIVATE);
