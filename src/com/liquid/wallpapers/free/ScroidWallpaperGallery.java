...
import com.liquid.wallpapers.free.core.communications.CommunicationChooseDialog;
import com.liquid.wallpapers.free.core.communications.CommunicationChooseDialog.CommunicationChosenListener;
import com.liquid.wallpapers.free.core.favourites.FavouriteListActivity;
import com.liquid.wallpapers.free.core.settings.SettingsActivity;
import com.liquid.wallpapers.free.core.wallpapers.WallpaperGalleryAdapter;
import com.liquid.wallpapers.free.core.wallpapers.WallpaperManager;
import com.liquid.wallpapers.free.core.wallpapers.WallpaperPreviewActivity;
...
public class ScroidWallpaperGallery extends Activity {
		...
		public void handleUncaughtException(Throwable ex) {
			if (ex instanceof WallpaperListReceivingException) {
				AlertDialogFactory.showErrorMessage(this.context,
						R.string.errorText, ex.getMessage(),
	...
	private class ShutDownAlertDialogOnClickListener implements
	...
	public ScroidWallpaperGallery() {
		...
		this.communicationDAO = DependencyInjector
				.getInstance(ICommunicationDAO.class);
	...
	private void handleOnCommunicationChosen(Communication communication) {
		...
		if (communication.getType().equals(Communication.Type.Email)) {
			...
			intent.putExtra(Intent.EXTRA_EMAIL,
					new String[] { communication.getValue() });
			intent.putExtra(Intent.EXTRA_SUBJECT,
					getBaseContext().getString(R.string.applicationName));
			intent.putExtra(
					Intent.EXTRA_TEXT,
					String.format(
							getBaseContext().getString(
									R.string.recommendEmailPattern),
							wallpaper.getWallpaperUrl()));
			intent.setType(Messages.getString("ScroidWallpaperGallery.0")); //$NON-NLS-1$

			this.startActivity(intent);
		} else if (communication.getType().equals(Communication.Type.Mobile)) {
			Intent intent = new Intent(Intent.ACTION_VIEW);
			intent.putExtra(
					Messages.getString("ScroidWallpaperGallery.1"), communication.getValue()); //$NON-NLS-1$
			intent.putExtra(
					Messages.getString("ScroidWallpaperGallery.2"), //$NON-NLS-1$
					String.format(
							getBaseContext().getString(
									R.string.recommendSmsPattern),
							wallpaper.getWallpaperUrl()));
			intent.setType(Messages.getString("ScroidWallpaperGallery.3")); //$NON-NLS-1$
	...
	private void initGallery() {
		...
		gallery.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Wallpaper wallpaper = (Wallpaper) parent
		...
		gallery.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				selectedWallpaper = (Wallpaper) wallpaperGalleryAdapter
						.getItem(position);
				...
				new Thread(new Runnable() {
					...
					@Override
					public void run() {
						preloadThumbs(wallpaperGalleryAdapter.getWallpapers(),
								(position + 1), 3);
					}
				}).start();
			}
	...
	private void onPickContactActivityResult(int resultCode, Intent data) {
		...
		Communication[] communications = this.communicationDAO
				.getCommunications(data.getData());

		if (communications.length < 1) {
			AlertDialogFactory.showInfoMessage(this, R.string.infoText,
					R.string.noCommunicationFoundInfoText);

			return;
		}

		CommunicationChooseDialog dialog = new CommunicationChooseDialog(this,
				communications, new CommunicationChosenListener() {
					 ...
					 */
					@Override
					public void onCommunicationChosen(
	...
	private synchronized void updateGalleryAdapter(Wallpaper[] wallpapers) {
		this.wallpaperGalleryAdapter = new WallpaperGalleryAdapter(this,
				wallpapers, this.wallpaperManager);
